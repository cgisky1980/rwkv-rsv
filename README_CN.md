...
| 并发 B | 信天翁 tok/s | rwkv-rsv tok/s | 提速 |
|---:|---:|---:|---:|
| 1（单流） | 94.2 – 99.0 | 94.1 – 97.2 | **持平**（3 轮交错均值 1.012×） |
| 8 | 465.4 | **547.8** | +17.7% |
| 16 | 806.0 | **979.4** | +21.5% |
...
> ⚠️ 该卡兼作桌面显示卡：**跨会话读数漂移可达 ±17%**（信天翁的 B=1 单档就在 94.2~99.0 之间游走）。
> **只有同会话交错 A/B 可信**，单次读数足以把结论反转 ±3%。

做到的途径：**int8 张量核**（`mma.m8n8k16.s8`；Turing 的 int8 张量核峰值是 fp16 档的 2×）跑在
