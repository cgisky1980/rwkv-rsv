...
| 主题 | 关键改动 / 结论 | 记录文档 |
|---|---|---|
| 后端统一抽象 | 引入 `ComputeBackend` trait，Vulkan/CUDA 双后端统一接口、自动选择 | [后端统一抽象设计.md](后端统一抽象设计.md)、[后端统一抽象-S1~S10完成记录.md](后端统一抽象设计.md)（S1–S10 为独立文件） |
| decode 优化（chain4） | `gemv_lowrank_chain4` 改 subgroup-per-row：无 shared/barrier，kernel ≈1.8×、e2e ≈+12% | [chain4_subgroup重构与head合成量化删除记录.md](chain4_subgroup重构与head合成量化删除记录.md) |
| decode 修复 | 修复 `gemv_rkv_stage1` ROWS=2→4 与 chain4 dispatch 网格错位两处静默错误 | [decode两跳dispatch错位修复与chain4融合回退记录.md](decode两跳dispatch错位修复与chain4融合回退记录.md) |
| head 量化路径 | 单独量化 head 端到端收益≈1%（淹没于热噪声），仅保留为全 int8 模型的一部分 | [head权重量化速度复测记录.md](head权重量化速度复测记录.md) |
| any4 移除 | 仅保留 fp16/int8 两路，删除 any4 shader/量化器/测试 | [去掉any4只留fp16与int8完成记录.md](去掉any4只留fp16与int8完成记录.md) |
| GPU self-loop | 批量提交：argmax 写回 host 缓冲，消除逐 token CPU↔GPU 同步 | [GPU-selfloop批量提交报告.md](GPU-selfloop批量提交报告.md) |
| GPU 平台对比 | fp16/int8 × Vulkan/CUDA 吞吐重测与测速方法学（热漂移归一化） | [decode速度平台对比复测记录.md](decode速度平台对比复测记录.md) |

### 6.2 量化策略性能对比（RTX 2080 Ti，RWKV-7 g1h-3B）

| 量化策略 | bit/权重 | 权重文件 | 省存 | 权重级 avg cos / rel | 端到端 Top-1一致率 | decode Vulkan | decode CUDA |
|---|---|---|---|---|---|---|---|
| fp16（基线） | 16 | 5.49 GB | — | 无损（参考） | 100% | 80.0 tok/s | 85.7 tok/s |
| int8（per-group, G=128） | 8 | 3.22 GB | 41% | 0.999820 / 0.6135% | **98.8%**（506/512） | 110.6 tok/s | 110.8 tok/s |
| any4 + nnq（已移除） | 4.35 | 2.07 GB | 62% | 0.995718 / 9.18%（k-means） | ~92.6%（真实多语言）/ 94.5%（单轨迹） | 114.7 tok/s（2026-08-06 旧测） | — |

> 说明：
> - decode 吞吐为 `memtest` + `SELFLOOP_ONLY=1` / `SELFLOOP_N=1000`（GPU 60°C 冷态，2026-08-09 重测）。int8 较 fp16 ≈ +38%（Vulkan）/ +29%（CUDA）。
> - any4 的 114.7 tok/s 为 2026-08-06 旧测（仅 Vulkan、冷态），与 fp16/int8 的 2026-08-09 读数非同窗口，仅作参考；any4 因精度不足（~90% 量级）与技术复杂度已于 2026-08-09 移除，当前仅保留 fp16/int8。
> - 权衡：int8 以 41% 省存 + 98.8% 近无损居中，是 fp16（无损但 5.49GB）与 any4（省 62% 但精度下降、已废弃）之间的推荐档。
> - 权威量化报告：[int8量化报告.md](int8量化报告.md)、[any4量化报告_final.md](any4量化报告_final.md)。